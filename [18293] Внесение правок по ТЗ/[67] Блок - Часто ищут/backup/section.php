...
<div class="main-wrapper flexbox flexbox--direction-row">
	<div class="section-content-wrapper <?=($bShowLeftBlock ? 'with-leftblock' : '');?> flex-1">
		...
		<?if ($section):?>
			...
			?>
			<? // Блок - разделы, как список тегов - "Часто ищут" ?> 
				<?
					// id инфоблока, в котором находятся разделы, в которых будем выводить теги
					$IBLOCK_ID = 43;
	
					// Получаем массив id-шников разделов, выбранных в пользовательском поле "Часто ищут" в конкретном разделе
					$rsSelectedSections = CIBlockSection::GetList(
					["SORT"=>"ASC"],
					["IBLOCK_ID"=>$IBLOCK_ID,"ID" =>$arResult["VARIABLES"]["SECTION_ID"]],
					false,
					["UF_LIST_TAG"],
				   );
					while($arSelectedTags = $rsSelectedSections->GetNext()){
						$arSelectedIDs = $arSelectedTags["UF_LIST_TAG"];
					...
					<?//Добавляем наш массив, в котором перечислены ID выбранных разделов  
		
					$arFilter = array('IBLOCK_ID' => $IBLOCK_ID, 'ACTIVE' => 'Y', 'ID' => $arSelectedIDs);
					$arSelect = array('ID', 'NAME', 'SECTION_PAGE_URL');
					$rsSection = CIBlockSection::GetTreeList($arFilter, $arSelect);
						while ($arSectionTags = $rsSection->GetNext()) {
						/*echo $arSectionTags['NAME'];*/
						/*echo $arSectionTags['ID'];*/
						/*echo $arSectionTags['SECTION_PAGE_URL'];*/
				?>
				   <div class="often-tags__item font_12 bordered rounded-4"><a href="<?=$arSectionTags['SECTION_PAGE_URL']?>"><?=$arSectionTags['NAME']?></a></div>
			   <?}?>
				...
				<?}?>
			<? // end Блок "разделы, как список тегов" ?> 
